...
class CleavagePatternSetEditorProvider {
  constructor(singlePattern = false) { this.singlePattern = singlePattern; }
  resolveCustomTextEditor(document, panel) {
    panel.webview.options = { enableScripts: true };
    panel.webview.html = editorHtml(this.singlePattern);
    let applyingEdit = false;

    const sendDocument = () => {
      try {
        const source = JSON.parse(document.getText());
        const value = this.singlePattern ? {cleavage_pattern_set:{name:'',patterns:[normalizePattern(source)]}} : normalizeDocument(source);
    ...
    panel.webview.onDidReceiveMessage(async message => {
      ...
      try {
        ...
        if (this.singlePattern && value.cleavage_pattern_set.patterns.length !== 1) throw new Error('A cleavage pattern file must contain exactly one pattern.');
        const savedValue = this.singlePattern ? value.cleavage_pattern_set.patterns[0] : value;
        const text = `${JSON.stringify(savedValue, null, 2)}\n`;
        const fullRange = new vscode.Range(document.positionAt(0), document.positionAt(document.getText().length));
        const edit = new vscode.WorkspaceEdit();
        edit.replace(document.uri, fullRange, text);
        applyingEdit = true;
        if (!await vscode.workspace.applyEdit(edit)) throw new Error('The document could not be updated.');
        applyingEdit = false;
...
function register(context) {
  for (const [viewType, singlePattern] of [[VIEW_TYPE, false], [PATTERN_VIEW_TYPE, true]]) {
    context.subscriptions.push(
      vscode.window.registerCustomEditorProvider(viewType, new CleavagePatternSetEditorProvider(singlePattern), {
...
function editorHtml(singlePattern = false) {
  return `<!doctype html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>${whiteLightTheme}
