...
function attach(panel, context, projectRoot, output) {
  ...
  panel.webview.onDidReceiveMessage(async message => {
    ...
    let observed;
    if (!['predictBatchPick', 'predictBatchCopy', 'predictBatchRun', 'predictBatchStop', 'predictSaveConfig', 'predictLoadConfig', 'predictLoadConfigFile'].includes(message.type)) return;
...
function client(defaults) {
  ...
  el('predictLoadConfigFile').onclick = () => vscode.postMessage({ type: 'predictLoadConfigFile' });
