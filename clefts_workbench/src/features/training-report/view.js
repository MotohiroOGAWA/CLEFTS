...
function client() {
  ...
  function renderChart(chart) {
    ...
    const color=lane=>splitColors[lane.split]||splitColors.all, dash=lane=>categoryDashes[categories.indexOf(lane.category)%categoryDashes.length];
    for(const lane of lanes){const item=document.createElement('span');item.style.color=color(lane);item.textContent=(chart.dimension?lane.category+' · ':'')+lane.split;legend.append(item);}card.append(legend);
  ...
  function renderCharts(){const query=el('reportFilter').value.trim().toLowerCase(),facet=el('reportFacet').value,include=el('reportPrecursor').checked,names=new Set(data.charts.filter(chart=>chart.dimension===facet).map(chart=>chart.metric));el('reportCharts').replaceChildren();let count=0;for(const chart of data.charts){const paired=without(chart.metric)?names.has(chart.metric.replace(/_without_precursor$/,'')):names.has(chart.metric+'_without_precursor');if(chart.dimension!==facet||(paired&&without(chart.metric)===include)||!labelMetric(chart.metric).toLowerCase().includes(query))continue;el('reportCharts').append(renderChart(chart));count++;}el('chartCount').textContent=count+' metric cards';}
