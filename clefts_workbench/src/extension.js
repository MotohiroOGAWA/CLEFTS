...
function workbenchHtml(config, fragmenterText, trainingConfig) {
  ...
  <nav><button class="tab" data-app="cleavage">Cleavage Pattern Set</button><button class="tab active" data-app="data">Data Preparation</button><button class="tab" data-app="training">Training</button><button class="tab" data-app="predict">Predict Spectrum</button></nav>
  ...
  <form id="trainingForm" data-app-panel="training" hidden>
    ...
    <section data-model-block="edge"><h2>Edge scorer and bounded candidate selection</h2><div class="model-notes"><p><b>Structural score:</b> embeds cleavage pattern/reaction/product IDs and the source/target molecular representations. Selected edges then attend only to atoms within the graph-distance limit.</p><p><b>Training budget:</b> intensity-weighted target groups and their path edges are proposed first; background edges fill the remaining per-sample budget. The union is capped by <i>Expensive edges / step</i>.</p><p><b>Prediction budget:</b> <i>Inference edges / depth</i> controls new edges at each cleavage depth, <i>Inference retained edges</i> carries survivors forward, and <i>Next-cleavage nodes</i> controls expansion.</p></div><div class="grid">${field('edgeFeatureDim','Edge feature dim','number')}${field('edgeCategoryDim','Category embedding dim','number')}${field('edgeAttentionHeads','Attention heads','number')}${field('attentionMaxGraphDistance','Atom graph distance','number')}${field('maxEdgesPerDepth','Inference edges / depth (CSV)','text')}${field('trainingEdgesPerSample','Candidate edges / sample','number')}${field('trainingZeroEdgeFraction','Background fraction','number','any')}${field('maxSamples','Samples / batch structure','number')}${field('maxEdgesPerStep','Expensive edges / step','number')}${field('maxRetainedEdges','Inference retained edges','number')}${field('maxNextCleavageCandidates','Next-cleavage nodes','number')}${field('edgeConditionInteractionDim','Condition interaction dim','number')}</div></section>
    <section data-model-block="ranking"><h2>Losses</h2><div class="model-losses"><span>Absolute edge ranking</span><span>+</span><span>Candidate selection</span><span>+</span><span>Formula intensity</span></div><div class="model-notes"><p><b>Absolute edge ranking:</b> orders formula/peak evidence sums by observed intensity and compares observed groups against unassigned background edges. High-intensity evidence receives greater weight.</p><p><b>Candidate selection:</b> supervises terminal fragment nodes, ion/unsaturation/radical states, every edge along a target path, and intermediate nodes that must be expanded. Selected-peak intensity coverage measures how much observed intensity remains reachable.</p><p><b>Formula intensity:</b> learns presence and relative abundance after equal-formula candidates are merged. Cosine similarity reports agreement between predicted and observed intensity vectors.</p></div><div class="grid">${field('rankingLossWeight','Ranking loss weight','number','any')}${field('rankingPairsPerEdge','Comparisons / edge','number')}${field('rankingIntensityThreshold','Intensity difference threshold','number','any')}</div></section>
...
function webviewScript() { return `
    ...
    document.querySelectorAll('[data-app]').forEach(button=>button.onclick=()=>{document.querySelectorAll('[data-app]').forEach(x=>x.classList.toggle('active',x===button));const app=button.dataset.app;document.getElementById('cleavageApp').hidden=app!=='cleavage';form.hidden=app!=='data';trainingForm.hidden=app!=='training';predictForm.hidden=app!=='predict';document.getElementById('dataActions').hidden=app!=='data';document.getElementById('appSubtitle').textContent=app==='cleavage'?'Cleavage Pattern Set Editor':app==='training'?'Fragment Tree Training':app==='predict'?'Predict Spectrum':'Fragment Tree Data Preparation';});
