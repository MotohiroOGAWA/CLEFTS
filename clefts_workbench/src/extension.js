...
function webviewScript() { return `
    ...
    function productAtomMarkup(a,l){const selected=productSelectedAtom===a.index||productBondStart===a.index;return \`<g data-mol-atom="\${a.index}" class="mol-atom \${selected?'selected ':''}\${productDeletedAtoms.has(a.index)?'deleted':''}" transform="translate(\${l.x(a)} \${l.y(a)})"><circle class="atom-hit" r="25"/><circle class="atom-label-bg" r="17"/><text class="atom-symbol product-map-number" text-anchor="middle" dominant-baseline="central">\${htmlEscape(productAtomMaps[a.index])}</text></g>\`}
    ...
    async function drawProductStructure(){productGraph=await chemistry('molecule',productSourcePayload());productAtomMaps={};productAtomOverrides={};productBondOverrides={};productDeletedAtoms=new Set();productAddedBonds=[];nextProductBondId=1;productBondStart=null;productSelectedAtom=null;productSelectedBond=null;productGraph.atoms.forEach((atom,index)=>productAtomMaps[atom.index]=atom.mapNumber||index+1);renderProduct()}
