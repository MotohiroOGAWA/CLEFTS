...
function session(context) {
  ...
  const value={dir,projectRoot:projects.root(context),jobs:[],owned:new Map(),listeners:new Set()};
...
function save(s,job){
  const file=path.join(s.dir,job.id+'.json');
  if(s.projectRoot){
    try{
      const paths=[job.outputDir,job.logPath,job.resultsPath].filter(Boolean);
      if(job.status!=='running'&&fs.statSync(job.outputDir,{throwIfNoEntry:false})?.isDirectory()){
        for(const entry of fs.readdirSync(job.outputDir,{withFileTypes:true}).slice(0,200)){
          if((entry.isFile()&&/\.(json|pt|msds|log|csv)$/.test(entry.name))||(entry.isDirectory()&&['train_structures','validation_structures'].includes(entry.name)))paths.push(path.join(job.outputDir,entry.name));
        }
      }
      const workflow=job.type==='preparation'?'data':job.type==='mol-training'?'molTraining':job.type.includes('prediction')?'prediction':'training';
      projects.store.track(s.projectRoot,paths,'run output',workflow,job.configurationId);
    }catch(error){job.trackingError=error.message;}
  }
...
async function startTraining(context,panel,output,root,python,config,buildArgs){
  ...
  const configurationId=projects.record(context,config.workflow==='mol'?'molTraining':'training',config,{reason:'run',label:config.experimentName||'Training run',base:root,jobId:id});
  ...
  const job={id,configurationId,projectRoot:projects.root(context),type:config.workflow==='mol'?'mol-training':'training',name:config.experimentName||path.basename(config.outputDir),status:'running',pid:child.pid,detached:process.platform!=='win32',startedAt:new Date().toISOString(),outputDir:path.resolve(root,config.outputDir),logPath,command:[python,...args],epochs:config.epochs};
...
function attach(panel,context,projectRoot,output,initialPage){
  let current=session(context);const post=data=>panel.webview.postMessage(data),update=()=>{const next=session(context);if(next!==current){current.listeners.delete(update);current=next;current.listeners.add(update);}post({type:'workbench/jobs',jobs:current.jobs});};current.listeners.add(update);const timer=setInterval(update,2000);panel.onDidDispose(()=>{clearInterval(timer);current.listeners.delete(update);});
...
function html(){return layout.sidebar()+`<p id="workbenchError" class="danger" role="alert"></p><div id="homePage" hidden>${projects.html()}${layout.home()}</div><div id="jobPage" hidden><section><h2>Training Jobs</h2><div id="allJobs"></div></section><section id="jobDetail" hidden><h2 id="jobTitle"></h2><p id="jobProgress"></p><progress id="jobProgressBar" max="100"></progress><div class="actions"><button id="jobStop" class="danger">Stop Training</button><button id="jobMetrics">Metrics</button><button id="jobOutput">Reveal Output</button></div><div id="jobMetricsPanel"><h3>Loss · Train / Validation</h3><div id="jobLossChart"></div><div id="jobActionMetrics" class="grid"></div></div><div id="jobIntermediateValidationPanel"><h3>Intermediate Validation · Loss / Optimizer Step</h3><p id="jobIntermediateValidation">No intermediate validation results yet.</p><div id="jobIntermediateValidationChart"></div></div><h3>Logs</h3><input id="logFilter" type="search" placeholder="Filter logs…" aria-label="Filter logs"><label class="check"><input id="logAuto" type="checkbox" checked>Auto Scroll</label><button id="logCopy">Copy</button><button id="logClear">Clear</button><pre id="jobLogs" role="log"></pre></section></div><div id="environmentPage" hidden><section><h2>Environment</h2><div class="actions"><button id="testEnvironment" class="primary">Test Environment</button><button id="openCleftsTerminal">Open Terminal</button></div><pre id="environmentDetails">Test Python, CLEFTS, PyTorch, RDKit and CUDA.</pre></section></div>`;}
function trainingHtml(){const fields={weightDecay:['Weight decay',0.01],gradientClip:['Gradient clip (0 disables)',1],absoluteWeight:['Absolute action loss weight',1],nextWeight:['Branching action loss weight',1],minimumPositiveWeight:['Minimum positive weight',0.05],negativeWeight:['Weak negative action weight',0.2],intensityWeight:['Fragment intensity loss weight',1],absoluteIntensityWeight:['Absolute intensity ranking weight',1],maxSamples:['Maximum simultaneous samples',128],seed:['Random seed',42],warmupSteps:['Warmup optimizer steps',100],validationIntervalSteps:['Validation interval (optimizer steps, 0 disables)',0],validationFraction:['Intermediate validation fraction (0 < value ≤ 1)',0.1],lrPatience:['LR reduction patience',3],earlyStoppingPatience:['Early stopping patience (0 disables)',10],minLr:['Minimum learning rate',0.000001]};return `<section><h2>Optimization and Loss</h2><label>Experiment Name<input name="experimentName" value="exp_main" placeholder="exp_main"></label><button type="button" id="trainingLossToggle" aria-expanded="true">Optimization and Loss Settings</button><div id="trainingLossFields"><div class="grid">${Object.entries(fields).map(([key,[label,value]])=>`<label>${label}<input name="${key}" type="number" min="${key==='maxSamples'?1:0}" max="${key==='validationFraction'?1:''}" step="${['maxSamples','seed','warmupSteps','validationIntervalSteps','lrPatience','earlyStoppingPatience'].includes(key)?1:'any'}" value="${value}"></label>`).join('')}</div><p class="muted">Set interval to 1000 and fraction to 0.1 to check a fixed 10% validation subset every 1000 optimizer steps. Full validation still runs at each epoch end.</p><label class="check"><input name="trainMolEncoder" type="checkbox">Update pretrained Mol Encoder weights</label><details><summary>How the losses work</summary><p>Absolute filtering learns precursor/action relevance and intensity ranking. Branching action loss learns independent valid cleavages with descendant-intensity weights. Validation generates spectra and records similarity distributions against original peaks. Post Model combines log intensity MSE with spectrum cosine loss. TensorBoard and detailed reports are written to the output directory. Training and validation require CUDA.</p></details></div></section>`;}

function css(){return layout.css()+projects.css();}
...
function client(){
  ...
  function chart(events,target='jobLossChart',series=['train_loss','validation_loss']){
    ...
    const max=Math.max(...values,0.001),count=Math.max(events.length-1,1),firstStep=events[0].global_step,lastStep=events.at(-1).global_step;const x=(e,j)=>target==='jobLossChart'?20+560*j/count:20+560*(e.global_step-firstStep)/Math.max(lastStep-firstStep,1);
    series.forEach((key,i)=>{const line=document.createElementNS(ns,'polyline');line.setAttribute('points',events.filter(e=>Number.isFinite(Number(e[key]))).map((e,j)=>x(e,j)+','+(155-130*Number(e[key])/max)).join(' '));line.setAttribute('fill','none');line.setAttribute('stroke',i?'var(--vscode-charts-orange,#d18616)':'var(--vscode-charts-blue,#3794ff)');line.setAttribute('stroke-width','2');svg.append(line);const label=document.createElementNS(ns,'text');label.setAttribute('x',String(20+i*220));label.setAttribute('y','175');label.setAttribute('fill','var(--vscode-foreground)');label.textContent=key;svg.append(label);});box.append(svg);
    if(target!=='jobLossChart')return;
  ...
  window.addEventListener('message',event=>{const m=event.data;if(m.type==='workbench/navigate')navigate(m.page);if(m.type==='project/state'&&m.restore){selected='';logs='';el('jobDetail').hidden=true;}if(m.type==='workbench/jobs'){jobs=m.jobs;table('recentJobs');table('allJobs');load();}if(m.type==='workbench/jobSelected'){selected=m.jobId;navigate('jobs');load();}if(m.type==='workbench/logs'&&m.job.id===selected){logs=m.text;renderLogs();el('jobDetail').hidden=false;el('jobTitle').textContent=m.job.name+' · '+m.job.status;el('jobMetricsPanel').hidden=m.job.type!=='training';el('jobIntermediateValidationPanel').hidden=m.job.type!=='training';el('jobMetrics').hidden=m.job.type!=='training';el('jobStop').disabled=m.job.status!=='running';const events=logs.split('\n').flatMap(line=>{try{const item=JSON.parse(line);return item.event==='epoch_end'?[item]:[];}catch{return [];}}),last=events.at(-1);chart(events);const intermediate=logs.split('\n').flatMap(line=>{try{const item=JSON.parse(line);return item.event==='intermediate_validation_end'?[item]:[];}catch{return [];}}),partial=intermediate.at(-1);chart(intermediate,'jobIntermediateValidationChart',['validation_loss']);el('jobIntermediateValidation').textContent=partial?'Step '+partial.global_step+' · '+partial.validation_samples+' / '+partial.total_validation_samples+' validation samples · Loss '+Number(partial.validation_loss).toFixed(6):'No intermediate validation results yet.';el('jobProgress').textContent=m.job.error||m.job.warning||m.job.trackingError||(m.job.type!=='training'?m.job.type+' · '+m.job.status:last?'Epoch '+last.epoch+' / '+m.job.epochs:'Waiting for epoch metrics…');if(last)el('jobProgressBar').value=100*last.current/last.total;else el('jobProgressBar').removeAttribute('value');}if(m.type==='workbench/environment'){el('environmentDetails').textContent=m.error||JSON.stringify(m.data,null,2);el('homeEnvironment').textContent=m.error?'CLEFTS environment unavailable: '+m.error:'CLEFTS environment ready · PyTorch '+m.data.torch+' · '+(m.data.cuda?'CUDA '+m.data.gpu:'CPU');}if(m.type==='workbench/error')el('workbenchError').textContent=m.error;});
...
function observeJob(context,info){
 const s=session(context),id=crypto.randomUUID();const configurationId=info.config?projects.record(context,info.workflow||info.type,info.config,{reason:'run',label:info.name,base:info.base||projects.root(context),jobId:id}):undefined;const job={id,configurationId,projectRoot:projects.root(context),name:info.name,type:info.type,status:'running',startedAt:new Date().toISOString(),outputDir:info.outputDir?path.resolve(info.base||process.cwd(),info.outputDir):s.dir,command:info.command||[],pid:info.child?.pid,logPath:path.join(s.dir,id+'.log'),detached:!!info.detached};
 fs.writeFileSync(job.logPath,'');if(info.child)s.owned.set(id,info.child);s.jobs.unshift(job);save(s,job);
 let finished=false;
 return {job,log:text=>fs.appendFileSync(job.logPath,text),finish:({code,signal,error,result,partial=false}={})=>{if(finished)return;finished=true;job.status=info.child?.cleftsCancelled||job.status==='cancelled'?'cancelled':error?'failed':signal?'cancelled':code===0||partial?'completed':'failed';job.exitCode=code;job.warning=partial?'Completed with some failed records. See the run log.':undefined;job.error=error;job.finishedAt=new Date().toISOString();if(result){job.resultsPath=path.join(s.dir,id+'.result.json');fs.writeFileSync(job.resultsPath,JSON.stringify(result));}s.owned.delete(id);save(s,job);}};
}
