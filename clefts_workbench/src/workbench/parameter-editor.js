...
function createParameterEditor(container, initial, kind = "training", editCleavagePatternSet) {
  ...
  const optional={action_model_params:{num_heads:4,max_roles:64},mol_encoder_params:{dropout:0}};
  ...
  function render(){
    ...
    for(const [key,value]of Object.entries(model)){
      ...
      const section=node('section'),heading=node('h2',key==='fragmenter_params'?'Fragmenter Parameters':title(key));section.append(heading);
      ...
      if(key==='fragmenter_params'||['mol_encoder_params','post_model_params'].includes(key)||(kind==='data'&&key!=='architecture')){const toggle=button('Advanced '+title(key),()=>{advanced.hidden=!advanced.hidden;toggle.setAttribute('aria-expanded',String(!advanced.hidden));});toggle.setAttribute('aria-expanded',String(!advanced.hidden));section.append(toggle);renderValue(model,key,advanced,'',true);section.append(advanced);}
...
function client(){
  ...
  window.addEventListener('message',event=>{const m=event.data;if(m.type==='parameters/loaded'&&editors[m.target]){editors[m.target].setValue(m.modelConfig);el(m.target+'ParameterFile').value=m.path;el(m.target+'ParameterStatus').textContent='Parameters loaded. Form values will be used for execution.';}if(m.type==='parameters/error')el(m.target+'ParameterStatus').textContent=m.error;if(m.type==='config'&&(m.config.modelConfig||m.config.fragmenterParams))editors.data.setValue(m.config.modelConfig||{fragmenter_params:m.config.fragmenterParams,symbols:m.config.symbols,max_node:m.config.maxNode,max_edge:m.config.maxEdge});if(m.type==='trainingConfig'&&m.config.modelConfig)editors.training.setValue(m.config.modelConfig);});
