...
function createParameterEditor(container, initial, kind = "training", editCleavagePatternSet, editAdductRuleSet) {
  ...
  const dataValue=value=>({fragmenter_params:clone(value.fragmenter_params||value),symbols:clone(value.symbols||value.mol_encoder_params?.symbols||initial.mol_encoder_params?.symbols||['C','N','O','P','S','F','Cl','Br','I']),max_unique_fragment_smiles:value.max_unique_fragment_smiles??-1,max_cleavage_combinations:value.max_cleavage_combinations??-1});
  ...
  const descriptions={
    ...
    max_unique_fragment_smiles:'Skip a source molecule as soon as its generated fragments exceed this many distinct canonical SMILES. The source molecule itself is not counted. -1 means unlimited.',
    max_cleavage_combinations:'Skip a source molecule as soon as its cleavage action search examines more than this many raw action combinations (every size from 1 to max_action_count), before RDKit runs them. -1 means unlimited.',
  ...
  function render(){
    ...
    for(const [key,value]of Object.entries(model)){
      ...
      if(kind==='training'&&['architecture','mol_encoder_checkpoint','fine_tuning','fragmenter_params','mol_encoder_params','adduct_type_strs','symbols','max_unique_fragment_smiles','max_cleavage_combinations'].includes(key))continue;
      ...
      if(kind==='data'&&['max_unique_fragment_smiles','max_cleavage_combinations'].includes(key))continue;
      if(kind==='data'&&key==='fragmenter_params'){renderValue(model,'max_unique_fragment_smiles',section,'');renderValue(model,'max_cleavage_combinations',section,'');}
...
function client(){
  ...
  const dataInitial={...defaultModel,symbols:initial.symbols||defaultModel.symbols||defaultModel.mol_encoder_params?.symbols,max_unique_fragment_smiles:initial.maxUniqueFragmentSmiles??defaultModel.max_unique_fragment_smiles,max_cleavage_combinations:initial.maxCleavageCombinations??defaultModel.max_cleavage_combinations};
  ...
  getConfig=()=>{const value=editors.data.getValue();return {...snapshot(form,'fragment-tree-data-preparation'),fragmenterParams:value.fragmenter_params,symbols:value.symbols,maxUniqueFragmentSmiles:value.max_unique_fragment_smiles,maxCleavageCombinations:value.max_cleavage_combinations};};
  ...
  window.addEventListener('message',event=>{const m=event.data;if(m.type==='training/checkpoint'&&!m.error&&[training.elements.resume.value,training.elements.fineTuneResume.value].includes(m.path)&&['resume','patterns'].includes(training.querySelector('input[name=trainingMode]:checked')?.value)){editors.training.setValue(m.modelConfig);training.elements.lr.value=m.lr;training.elements.weightDecay.value=m.weightDecay;for(const [key,value]of Object.entries(m.trainingSettings||{})){const input=training.elements[key];if(input){if(input.type==='checkbox')input.checked=value;else input.value=value;}}const fine=m.modelConfig.fine_tuning;if(fine){training.elements.fineTuneCheckpoint.value=fine.source_checkpoint||fine.base_checkpoint||fine.checkpoint_path||'';training.elements.adapterWidth.value=fine.adapter_width??fine.width??8;}}if(m.type==='parameters/loaded'&&editors[m.target]&&!(m.target==='training'&&(training.querySelector('input[name=trainingMode]:checked')?.value==='resume'||training.querySelector('input[name=trainingMode]:checked')?.value==='patterns'&&training.elements.fineTuneResume.value))){editors[m.target].setValue(m.modelConfig);const status=el(m.target+'ParameterStatus');if(status)status.textContent='Loaded '+(m.path?.split(/[\\/]/).at(-1)||'fragmenter config')+'.';}if(m.type==='parameters/saved'){const status=el(m.target+'ParameterStatus');if(status)status.textContent='Saved '+(m.path?.split(/[\\/]/).at(-1)||'fragmenter config')+'.';}if(m.type==='parameters/error'){const status=el(m.target+'ParameterStatus');if(status)status.textContent=m.error;}if(m.type==='config'&&(m.config.modelConfig||m.config.fragmenterParams))editors.data.setValue(m.config.modelConfig||{fragmenter_params:m.config.fragmenterParams,symbols:m.config.symbols,max_unique_fragment_smiles:m.config.maxUniqueFragmentSmiles,max_cleavage_combinations:m.config.maxCleavageCombinations});if(m.type==='trainingConfig'&&m.config.modelConfig)editors.training.setValue(m.config.modelConfig);});
