...
class FragmentSpectrumGenerator(ModelBase):
    ...
    Intensities are computed from graph-wise emission probabilities:
        fragment intensity = p_emit / max_p_emit
        precursor intensity = p_precursor_emit / max_p_emit
    """
    ...
    def _compute_max_p_emit(
        ...
        Tensor
            ...
            Each value is the max emission probability within the same sample graph.
        """

        sample_tree_batch = probability_output.sample_tree_batch

        p_emit = flow_probabilities.p_emit
        p_precursor_emit = flow_probabilities.p_precursor_emit
        ...
        if p_precursor_emit.numel() > 0:
            ...
            precursor_graph_index = graph_index_by_node[precursor_root_node]
            # [P]

            max_p_emit_by_graph.scatter_reduce_(
                dim=0,
                index=precursor_graph_index,
                src=p_precursor_emit,
                reduce="amax",
                include_self=True,
            )
    ...
    def _build_precursor_annotations(
        ...
        *,
        ...
        graph_index_by_precursor = torch.repeat_interleave(
            torch.arange(row_counts.numel(), dtype=torch.long, device=precursor_pathway_seq.device),
            row_counts,
        )
        ...
        for pathway_index in range(int(precursor_pathway_seq.size(0))):
            probability = float(flow_probabilities.p_precursor_emit[pathway_index].detach().cpu().item())
            intensity = probability / max_emit

            if intensity < self.min_peak_intensity:
                continue

            graph_index = int(graph_index_by_precursor[pathway_index].detach().cpu().item())
            sample_id = int(kept_sample_ids[graph_index].detach().cpu().item())
            batch_node_index = int(precursor_pathway_seq[pathway_index, 0].detach().cpu().item())
            ...
            adduct_type_index = int(structure.sample_adduct_type_index[sample_id].detach().cpu().item())
            adduct = self.probability_model.main_adduct_types[adduct_type_index]
    ...
    def _build_fragment_annotations(
        ...
        *,
        ...
        for batch_node_index in range(int(flow_probabilities.p_emit.size(0))):
            node_probability = float(flow_probabilities.p_emit[batch_node_index].detach().cpu().item())
            node_intensity = node_probability / max_emit
            ...
            for ion_index, unsaturation_index, radical_index, adduct_probability in adduct_combinations:
                if adduct_probability < self.min_fragment_ion_probability:
                    continue

                ion_adduct = self.probability_model.ion_flat_candidates[ion_index, 1]
                unsaturation_adduct = self.probability_model.unsaturation_flat_candidates[unsaturation_index, 1]
                radical_adduct = self.probability_model.radical_flat_candidates[radical_index, 1]
    ...
    def _resolve_formula_mz(
        ...
        """Resolve formula and m/z from smiles + adduct."""
    ...
    def _normalize_intensity(
        ...
        return [
            GeneratedSpectrumPeak(
                mz=peak.mz,
                intensity=100.0 * peak.intensity / max_intensity,
